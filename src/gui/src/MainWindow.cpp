...
#if !defined(INPUTLEAP_MINIMAL_BUILD)
    ...
    m_pLogWindow(nullptr)
{
    ...
    ui_->m_pCheckBoxAutoConfig->setChecked(appConfig.minimalMode() ? false : appConfig.autoConfig());
    m_SuppressAutoConfigWarning = false;
...
void MainWindow::open()
{
    ...
    if (!appConfig().minimalMode() && !appConfig().autoConfigPrompted()) {
        promptAutoConfig();
    }
...
void MainWindow::appendLogInfo(const QString& text)
{
    if (appConfig().logLevel() >= 3) {
        if (m_pLogWindow) m_pLogWindow->appendInfo(text);
    }
}

void MainWindow::appendLogDebug(const QString& text) {
    if (appConfig().logLevel() >= 4) {
        if (m_pLogWindow) m_pLogWindow->appendDebug(text);
    }
}

void MainWindow::appendLogError(const QString& text)
{
    if (m_pLogWindow) m_pLogWindow->appendError(text);
}
...
void MainWindow::appendLogRaw(const QString& text)
{
    for (const auto& line : text.split(QRegularExpression("\r|\n|\r\n"))) {
        if (!line.isEmpty()) {
            if (m_pLogWindow) m_pLogWindow->appendRaw(line);
...
void MainWindow::start_cmd_app()
{
    ...
    args << "-f" << "--no-tray" << "--debug"
         << (appConfig().minimalMode() ? QStringLiteral("ERROR") : appConfig().logLevelText());
    ...
    if (!appConfig().minimalMode() && m_ServerConfig.enableDragAndDrop()) {
    ...
    if (m_pLogWindow) m_pLogWindow->startNewInstance();
...
#if defined(Q_OS_WIN)
    if (appConfig().autoDisplaySwitch()) {
        args << "--enable-auto-display-switch"
             << "--target-client-name" << appConfig().targetClientName()
             << "--target-local-display" << appConfig().targetLocalDisplay();
    }
#endif
