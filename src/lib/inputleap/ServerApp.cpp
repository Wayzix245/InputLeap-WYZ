...
void ServerApp::handle_display_client_connected(const Event& event)
{
#if SYSAPI_WIN32
    if (display_controller_) {
        display_controller_->clientConnected(event.get_data_as<Server::ScreenConnectedInfo>().m_screen);
    }
#else
    (void) event;
#endif
...
void ServerApp::handle_display_client_disconnected(const Event& event)
{
#if SYSAPI_WIN32
    if (display_controller_) {
        display_controller_->clientDisconnected(event.get_data_as<Server::ScreenConnectedInfo>().m_screen);
    }
#else
    (void) event;
#endif
...
#if SYSAPI_WIN32
void ServerApp::remove_display_handlers()
{
    if (display_handler_target_ != nullptr) {
        m_events->remove_handler(EventType::SERVER_CONNECTED, display_handler_target_);
        m_events->remove_handler(EventType::SERVER_CLIENT_DISCONNECTED, display_handler_target_);
        display_handler_target_ = nullptr;
    }
...
#if SYSAPI_WIN32
    remove_display_handlers();
    if (args().enable_auto_display_switch && !args().target_client_name.empty() &&
        !args().target_local_display.empty()) {
        display_controller_ = std::make_unique<MSWindowsDisplayController>(
            args().target_client_name, args().target_local_display);
        display_handler_target_ = primaryClient->get_event_target();
        m_events->add_handler(EventType::SERVER_CONNECTED, display_handler_target_,
                              [this](const auto& e) { handle_display_client_connected(e); });
        m_events->add_handler(EventType::SERVER_CLIENT_DISCONNECTED, display_handler_target_,
                              [this](const auto& e) { handle_display_client_disconnected(e); });
    }
#endif
