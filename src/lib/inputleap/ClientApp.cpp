...
{
    static const double delays[] = {1.0, 2.0, 5.0, 10.0, 30.0};
    const std::size_t count = sizeof(delays) / sizeof(delays[0]);
    const std::size_t index = m_restartAttempt < count ? m_restartAttempt : count - 1;
    ++m_restartAttempt;
    return delays[index];
