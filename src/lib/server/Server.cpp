...
Server::Server(
	...
	m_enableClipboard(!args.m_minimalMode),
...
Server::removeClient(BaseClientProxy* client)
{
    ...
    const std::string disconnectedName = getName(client);
    ...
    if (client != m_primaryClient) {
        Server::ScreenConnectedInfo info{disconnectedName};
        m_events->add_event(EventType::SERVER_CLIENT_DISCONNECTED,
                            m_primaryClient->get_event_target(),
                            create_event_data<Server::ScreenConnectedInfo>(info));
    }
